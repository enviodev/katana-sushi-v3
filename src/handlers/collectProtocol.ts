// CollectProtocol: same shape as pool Collect with no tickLower/Upper.
// Subgraph creates a Collect (not a separate ProtocolCollect) — match that.
import { indexer, type Collect } from "envio";
import { getChainConfig } from "../utils/chains.js";
import { FACTORY_ADDRESS, ONE_BI, ZERO_BI } from "../utils/constants.js";
...
indexer.onEvent(
  { contract: "UniswapV3Pool", event: "CollectProtocol", wildcard: true },
  async ({ event, context }) => {
    ...
    const collect: Collect = {
      ...
      pool_id: pool.id,
      // For protocol fee collects, the recipient is the canonical "owner" in
      // the subgraph collect entity (mirrors handleProtocolCollect).
      owner: event.params.recipient.toLowerCase(),
      amount0: collected0,
      amount1: collected1,
